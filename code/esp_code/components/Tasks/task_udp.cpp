...
extern "C" void task_udp(void *arg)
{
    ...
    for (;;) {
        ...
        if(core_time - send_time > 1'000'000){
            sprintf(message_buff,"%llu, %1.3f, %1.3f, %1.3f, %1.3f, %1.3f, %1.3f, %1.3f, ", 
								esp_timer_get_time(), val.tof.tof1, val.tof.tof2, val.tof.tof3, val.tof.tof4, val.orient.heading, val.log.voltage, val.log.gyro_freq);
			// printf("Message: %s\n", message_buff);
            // printf("It took: %d\n", val.log.hz_gyro);
            // printf("Voltage: %f\n", val.log.voltage);
            // printf("Freq gyro: %f\n", val.log.gyro_freq);
