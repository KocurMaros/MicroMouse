...
static calibration_t cal = { .mag_offset = { .x = 0.0, .y = 0.0, .z = 0.0 },
...
void calibrate_accel(void)
{
	...
	printf("    .accel_offset = {.x = %f, .y = %f, .z = %f},\n    .accel_scale_lo = {.x = %f, .y = %f, .z = %f},\n    .accel_scale_hi = {.x = %f, .y = %f, .z "
		   "= %f},\n",
		   offset.x, offset.y, offset.z, scale_lo.x, scale_lo.y, scale_lo.z, scale_hi.x, scale_hi.y, scale_hi.z);
}
...
{
	vector_t v_min = { .x = 9.9e99, .y = 9.9e99, .z = 9.9e99 };
	vector_t v_max = { .x = -9.9e99, .y = -9.9e99, .z = -9.9e99 };

	const int NUM_MAG_READS = 7000;

	init_imu();

	ESP_LOGW(TAG, "Rotate the magnometer around all 3 axes, until the min and max values don't change anymore.");

	printf("n    x        y        z      min x     min y     min z     max x     max y     max z\n");
	for (int i = 0; i < NUM_MAG_READS; i += 1) {
		...
		printf("%d   %0.3f    %0.3f    %0.3f    %0.3f   %0.3f   %0.3f   %0.3f   %0.3f   %0.3f       \n", i, vm.x, vm.y, vm.z, v_min.x, v_min.y, v_min.z,
			   v_max.x, v_max.y, v_max.z);

		vTaskDelay(10 / portTICK_PERIOD_MS);
	}

	vector_t v_avg = { .x = (v_max.x - v_min.x) / 2.0, .y = (v_max.y - v_min.y) / 2.0, .z = (v_max.z - v_min.z) / 2.0 };
	float avg_radius = (v_avg.x + v_avg.y + v_avg.z) / 3.0;
	vector_t v_scale = { .x = avg_radius / v_avg.x, .y = avg_radius / v_avg.y, .z = avg_radius / v_avg.z };
