...
const char *CLK_RNG[] = { "0 (Internal 20MHz oscillator)",
...
void mpu9250_print_settings(void)
{
	...
	ESP_LOGI(TAG, "  --> Accel enabled (x, y, z): (%s, %s, %s)", YN(accel_ps.x), YN(accel_ps.y), YN(accel_ps.z));
	ESP_LOGI(TAG, "  --> Gyro enabled (x, y, z): (%s, %s, %s)", YN(gyro_ps.x), YN(gyro_ps.y), YN(gyro_ps.z));
}
...
{
	const char *FS_RANGE[] = { "+250 dps (0)", "+500 dps (1)", "+1000 dps (2)", "+2000 dps (3)" };
